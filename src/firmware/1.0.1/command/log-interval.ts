...
export const parseLogInterval = (text: string) => {
  const frame = 'log interval'
  // Incomplete Frame
  if (text.length < LOG_INTERVAL_FRAME_LENGTH) { return { frame, raw: text, error: 'frame incomplete' } }
  // Get Log Interval
  const raw = text
  const li = raw.replace(LOG_INTERVAL_START, '')
  // Incorrect Log Interval
  if (isNaN(Number(li))) { return { frame, raw, error: `${li} is not a number` } }
