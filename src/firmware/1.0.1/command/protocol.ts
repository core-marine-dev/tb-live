...
export const parseProtocols = (text: string) => {
  const frame = 'listenning protocols'
  // Incomplete Frame
  if (text.length < PROTOCOLS_FRAME_LENGTH) { return { frame, raw: text, error: 'frame incomplete' } }
  // Get Protocols
  const raw = text
  const lm = raw.replace(PROTOCOLS_START, '')
  // Incorrect Protocols
  if (isNaN(Number(lm))) { return { frame, raw, error: `${lm} is not a number` } }
